	...
	for (TTuple<FGameplayTag, FGameplayTag> Pair : GameplayTags.DamageTypesToDebuffs)
	{
		...
		if (TypeDamage > -.5f) // .5 padding for floating point imprecision
		{
			...
			if (bDebuff)
			{
				...
				UAuraAbilitySystemLibrary::SetShouldHitReact(ContextHandle, false);
                                              ...
                                              FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	// Capturing Source Armor Penetration
	...
	// Capturing ArmorPenetration on DamageCalculationCoefficients Curve
	const FRealCurve* ArmorPenetrationCurve = CharacterClassInfo->DamageCalculationCoefficients->FindCurve(FName("ArmorPenetration"), FString());
	const float ArmorPenetrationCoefficient = ArmorPenetrationCurve->Eval(SourcePlayerLevel);

	// Capturing EffectiveArmor on DamageCalculationCoefficients Curve
	...
	// Capturing CriticalHitResistance on DamageCalculationCoefficients Curve
	const FRealCurve* CriticalHitResistanceCurve = CharacterClassInfo->DamageCalculationCoefficients->FindCurve(FName("CriticalHitResistance"), FString());
	const float CriticalHitResistanceCoefficient = CriticalHitResistanceCurve->Eval(TargetPlayerLevel);
	
	// Capturing Target Critical Hit Resistance
	float TargetCriticalHitResistanceChance = 0.f;
	ExecutionParams.AttemptCalculateCapturedAttributeMagnitude(DamageStatics().CriticalHitResistanceDef, EvaluationParameters, TargetCriticalHitResistanceChance);
	TargetCriticalHitResistanceChance = FMath::Max<float>(TargetCriticalHitResistanceChance, 0.0f);
	
	// Capturing Source Critical Hit Chance
