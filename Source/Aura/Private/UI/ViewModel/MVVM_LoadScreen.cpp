...
{
	LoadSlots[Slot]->OnWidgetSwitcherIndexSet.Broadcast(1);
}
...
void UMVVM_LoadScreen::SelectSlotButtonPressed(int32 Slot)
{
	...
	{
		// Enable the button if Slot's SlotIndex is different than Current SlotIndex
		LoadSlot.Value->OnSelectSlotButtonEnabled.Broadcast(LoadSlot.Key != Slot);
	}
...
void UMVVM_LoadScreen::DeleteButtonPressed()
{
	if (IsValid(SelectedSlot))
	{
		...
		SelectedSlot->OnSelectSlotButtonEnabled.Broadcast(true);
	}
}
...
void UMVVM_LoadScreen::LoadData()
{
	...
	for (const TTuple<int32, UMVVM_LoadSlot*> LoadSlot : LoadSlots)
	{
		const UAuraSaveGame* SaveObject = AuraGameMode->GetSaveSlotData(LoadSlot.Value->GetLoadSlotName(), LoadSlot.Key);
